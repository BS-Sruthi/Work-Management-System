package com.sruthi.thiranx.features.task.status;
...
import com.sruthi.thiranx.data.dto.TaskStatusHistory;
...
class TaskStatusUpdateModel {
    ...
    private void notifyCreator(Task task, Task.TaskStatus oldStatus,
        ...
        alert.setEmployeeId(creatorId);
        alert.setTaskId(task.getId());
        alert.setType(Alert.NotificationType.STATUS_UPDATED);
        String who = (changedBy == null || changedBy.getName() == null) ? "an employee" : changedBy.getName();
        alert.setMessage("Task '" + task.getTitle() + "' status changed from "
                + oldStatus + " to " + newStatus + " by " + who);
        ThiranXDB.getInstance().addNotification(alert);
    }
}
